...
public class ClusterCompareView extends JPanel implements MouseListener, MouseMotionListener
{
	...
	public void paintComponent( Graphics g )
	{
		...
		int iMaxK = m_ClusterTable.m_iMaxK;
...
//		int iBorderTop = 10;
//		int iBorderBottom = 10;
//		int iBorderLeft = 10;
//		int iBorderRight = 10;
		int iGapX = Math.min(5, this.getWidth() / (2 * iMaxK)); // horizontal gap between bars
		int iBarW = (this.getWidth() / iMaxK) - iGapX;  // bar (column) width
		int iGapY = Math.min(5, this.getHeight() / (2 * iMaxK)); // vertical gap between bars
		double dActualHeight = this.getHeight() - iGapY * iMaxK; // total height of bars in each column without considering the gaps 
		
		if (m_iSelectedK > 0 && m_iSelectedK <= iMaxK)
		{
			// draw the bars for the neighbor Ks
			try
			{
				// the number of points shared between all pair of clusters for two differnt Ks
				int pairCompare[][] = new int[iMaxK+1][iMaxK+1];

				for (int iCompareK = 1; iCompareK < iMaxK; iCompareK++)
				{
					//if (iCompareK == m_iSelectedK)
					//	continue;
					...
					// sort the clusters for iCompareK based on the elements sharing with the m_iSelectedK
					Integer sortedBySelected[] = new Integer[iCompareK];
					...
					for (int iCmp = 0; iCmp < iCompareK; iCmp++)
					{
						...
						for (int iSel = 0; iSel < m_iSelectedK; iSel++)
						{
							...
							if (pairCompare[iSelSorted][iCmpSorted] == 0)
								continue;
							double dBarH = (dActualHeight * pairCompare[iSelSorted][iCmpSorted]) / iDataSize;
							Color colFill = new Color(colorTable[iSelSorted % colorTable.length]);
							Color colBorder = colFill;
							if (iCompareK != m_iSelectedK)
								colFill = new Color(colFill.getRed()/2 + 128, colFill.getGreen()/2 + 128, colFill.getBlue()/2 + 128);
							g2D.setColor(colFill);
							g2D.fillRect(iBarX, (int)dBarY, iBarW, (int)Math.ceil(dBarH));
							g2D.setColor(colBorder);
							g2D.drawRect(iBarX, (int)dBarY, iBarW, (int)Math.ceil(dBarH));
							dBarY += dBarH;
						}
						dBarY += iGapY;
					}
					...
					if (iCompareK == m_iSelectedK)
					{
						...
						g2D.drawRect(iBarX - 2, 0, iBarW + 4, this.getHeight() - 1);
					}

				}// for (iCompare 1..iMaxK
			}
			catch (Exception e)
			{
				e.printStackTrace();
		...
		try
		{
			// draw the bars for the selected K and the gray bars for far neighours
...
//	      g2d.fillRect(99,99,199,119);
	...
	{
		m_iSelectedK = ((m_ClusterTable.m_iMaxK * e.getX()) / this.getWidth()) + 1;
		if (e.getClickCount() == 1)
		{
			m_iMouseClickX = e.getX();
			m_iMouseClickY = e.getY();
			repaint();
