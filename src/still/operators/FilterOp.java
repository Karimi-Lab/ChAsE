...
public class FilterOp extends Operator {
	...
	public void updateFunction() {
		...
		if (selected_dimension < input.columns())
		{
			if( use_selection_dimension ) {
				...
				if( 	input.getColType( selected_dimension) == ColType.CATEGORICAL || 
						...
						input.getColType( selected_dimension) == ColType.METADATA) {
	...
	public class FilterView extends OperatorView implements ItemListener, ListSelectionListener {
		...
		public void resetGUI() {
			...
			//nonAttributeDims = Operator.getInverseDimValues(operator.input, ColType.ATTRIBUTE);
			// [HY] quick solution to be able to filter based on attributes such as selection.
			//      Should this be made as an option through a selection box?
			nonAttributeDims = Operator.getInverseDimValues(operator.input, ColType.METADATA);
			...
			else if(operator.input.getColType( selected_dimension) == ColType.METADATA) {
				
				for( String s : operator.input.getMetaData( selected_dimension ) ) {
			...
			binList.setEnabled( ( operator.input.getColType(selected_dimension) == ColType.NUMERIC || operator.input.getColType(selected_dimension) == ColType.ATTRIBUTE) && (use_bins || unique_values == null) );			
		}
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if( ! e.getValueIsAdjusting() ) {
				...
				if( e.getSource() == this.dimensionList && ((JList)e.getSource()).getSelectedIndex() != -1 ) {
					...
					else if (this.operator.input.getColType( selected_dimension) == ColType.METADATA) {
						
						for( String s : this.operator.input.getMetaData( selected_dimension ) ) {
