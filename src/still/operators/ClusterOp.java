...
//		}
	...
	public int rows()
	{
		...
		for (int c = 0; c  < m_ClusterTable.m_iCurrentK; c++)
		{
...
//		saveString += cullColorBy;
		saveString += m_ClusterTable.m_iCurrentK;
		return saveString;
	}
	...
	public class ClusterFunction extends AppendFunction implements Function, Serializable
	{
		...
		public double compute(int row, int col) 
		{
			...
			int iSelectedPoints = 0;
			int iRow = -1;
			for (int c = 0; c  < m_ClusterOp.m_ClusterTable.m_iCurrentK; c++)
			{
...
//		public double[] invert( Map map, int row, int col, double value ) ...
	...
	public class ClusterView extends OperatorView implements ChangeListener, ListSelectionListener
	{
	    ...
	    ScreePlot 	m_AllClustersQualityScreePlot = null;
	    ScreePlot	m_CurrClusterQualityScreePlot = null;
		...
		void buildNumClustersSlider()
		{
			...
			m_NumClustersSlider = new JSlider( 	JSlider.HORIZONTAL, 1, ((ClusterOp)operator).m_ClusterTable.m_iMaxK, 1);
			...
			m_NumClustersSlider.setValue(((ClusterOp)operator).m_ClusterTable.m_iCurrentK);
		...
		public void buildClusterSelectionList()
		{
			...
			String clusterListLabel[] = new String[op.m_ClusterTable.m_iCurrentK];
			int iSelectedClusters[] = new int[op.m_ClusterTable.m_iCurrentK];
			int iSelected = 0;
			for (int c = 0; c < op.m_ClusterTable.m_iCurrentK; c++)
			{
		...
		{
			// create the histogram that shows the per K quality measures for all K
			ClusterOp op = (ClusterOp)operator;
			int iQuality = op.m_ClusterTable.m_Quality.ordinal();
			ArrayList<Double> univarQuant = new ArrayList<Double>();
			ArrayList<String> univarNames = new ArrayList<String>();
			for(int i = 0; i < op.m_ClusterTable.m_iMaxK; i++)
			{
				double fVal = 0;
				if (op.m_ClusterTable.m_AllClusterInfoArray != null && i < op.m_ClusterTable.m_AllClusterInfoArray.length)
				{
					fVal = op.m_ClusterTable.m_AllClusterInfoArray[i].m_QualityMeasuresArray[iQuality];
				}
				univarQuant.add(new Double(fVal));
				univarNames.add(Integer.toString(i+1));
			}
			
			m_AllClustersQualityScreePlot = new ScreePlot(univarQuant, univarNames, false, 
											new Comparator<Double>() {public int compare(Double o1, Double o2) {return 0;}}, // no need to sort
											null);
			m_AllClustersQualityScreePlot.cutoff = op.m_ClusterTable.m_iCurrentK - 1;
			m_AllClustersQualityScreePlot.useDimensionNames = true;
			m_AllClustersQualityScreePlot.isCutoffLeft = true;
			//scree.addLogStateCheckbox( jcb );
			m_AllClustersQualityScreePlot.addChangeListener( this );
			
			
			// create the histogram that shows per cluster quality measures for current selected K
			univarQuant.clear();
			if (op.m_ClusterTable.m_AllClusterInfoArray != null && op.m_ClusterTable.m_iCurrentK <= op.m_ClusterTable.m_AllClusterInfoArray.length)
			{
				ClusterTable.AllClusterInfo currInfo = op.m_ClusterTable.m_AllClusterInfoArray[op.m_ClusterTable.m_iCurrentK - 1];
				for(int i = 0; i < op.m_ClusterTable.m_iMaxK; i++)
				{
					double fVal = 0;
					if (i < op.m_ClusterTable.m_iCurrentK)
					{
						fVal = currInfo.m_ClusterInfoArray[i].m_QualityMeasuresArray[iQuality];
					}
					univarQuant.add(new Double(fVal));
			...
			m_CurrClusterQualityScreePlot = new ScreePlot(univarQuant, univarNames, false, 
											new Comparator<Double>() {public int compare(Double o1, Double o2) {return 0;}}, // no need to sort
											null);
			//m_CurrClusterQualityScreePlot.cutoff = op.m_ClusterTable.m_iCurrentK - 1;
			m_CurrClusterQualityScreePlot.useDimensionNames = true;
			//m_CurrClusterQualityScreePlot.isCutoffLeft = true;
			//m_CurrClusterQualityScreePlot.addLogStateCheckbox( jcb );
			m_CurrClusterQualityScreePlot.addChangeListener( this );
		}
		...
		public void buildGui()
		{
			...
			JPanel clusterQualityPanel = new JPanel(new BorderLayout(5, 5));
			measurePanel.add(clusterQualityPanel, BorderLayout.CENTER);
			JPanel clusterScreePanel = new JPanel(new GridLayout(1,2,0,0));
			clusterQualityPanel.add(clusterScreePanel, BorderLayout.CENTER);
			buildClusterQualityScreePlot();
			clusterScreePanel.add(m_AllClustersQualityScreePlot);
			clusterScreePanel.add(m_CurrClusterQualityScreePlot);
			
			JPanel typePanel		= new JPanel( new GridLayout(1,2,0,0) );
			clusterQualityPanel.add(typePanel, BorderLayout.NORTH);
			...
			clusterQualityPanel.add(calcAllPanel, BorderLayout.SOUTH);
...
//			typePanel.add(clusterListPane);
			...
			if (op.m_ClusterTable != null)
			{
				...
				m_CCView.setSelectedK(op.m_ClusterTable.m_iCurrentK);
			}
		...
		void calculateAll()
		{
			...
			{
				op.m_ClusterTable.calculateAll(op.m_ClusterTable.m_iMaxK);
			}
			if (op.m_ClusterTable.m_bSelectBestK)
			{
				int iBestNumClusters = op.m_ClusterTable.getBestK(op.m_ClusterTable.m_Quality);
				if (iBestNumClusters != op.m_ClusterTable.m_iCurrentK)
				{
					op.m_ClusterTable.m_iCurrentK = iBestNumClusters;
				}
			}
		}
		...
		public void stateChanged(ChangeEvent e)
		{
			...
			if (e.getSource() == m_NumClustersSlider) {
			    if (!m_NumClustersSlider.getValueIsAdjusting()) {
			    	op.m_ClusterTable.m_iCurrentK = (int)m_NumClustersSlider.getValue();
			...
			else if( e.getSource() == m_AllClustersQualityScreePlot)
			{
		    	op.m_ClusterTable.m_iCurrentK = m_AllClustersQualityScreePlot.cutoff+1;
		    	bClustersChanged = true;
			}
			else if (e.getSource() == m_CCView)
			{
				op.m_ClusterTable.m_iCurrentK = m_CCView.m_iSelectedK;
				bClustersChanged = true;
			}
