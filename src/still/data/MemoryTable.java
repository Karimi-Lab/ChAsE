...
public class MemoryTable implements Table, Serializable
{
	...
	public String getColName( int dim ) {
		if (columnNames != null && dim >= 0 && columnNames.length > dim)
	...
	public MemoryTable( double[][] initialTable, ColType[] types, String[][] categories, String[][] metadata, String[] colNames ) {
...
//	}
	...
	public String getMetaData(int dim, int point_idx) {
		...
		if (metadata[dim] != null && metadata[dim].length > point_idx)
			return metadata[dim][point_idx];
		return null;
	}
