...
public class FloatIndexer {
	...
	// returns the reverse index of the sorted values (i.e. from high to low)
	public static int[] sortFloatsRev( double[] vals ) {
		int[] result = sortFloats(vals);
		for (int i1 = 0; i1 < result.length / 2; i1++)
		{
			int i2 = result.length - i1 - 1;
			int tmp = result[i1];
			result[i1] = result[i2];
			result[i2] = tmp;
		}
