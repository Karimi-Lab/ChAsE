...
public class Binner {
	...
	public void bin( ) {
		...
		if( use_bins ) {
			...
			if ( inner_table.getColType(bin_dimension) != Table.ColType.NUMERIC && 
				...
				if (inner_table.getColType(bin_dimension) == Table.ColType.METADATA)
				{
					for( int i = 0; i < inner_table.getMetaData(bin_dimension).length; i++ ) {
	...
	public String[] getBinStrings() {
		...
		if( getUseBins() ) {
			...
			else if (inner_table.getColType( bin_dimension ) == Table.ColType.METADATA)
			{
				return inner_table.getMetaData( bin_dimension );
			}
			else {
