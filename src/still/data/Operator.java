...
//			return matrix.toArray2()[point_idx][dim];
	...
	public String[] getMetaData( int dim ) {
		ArrayList<Integer> colsamp = map.getColumnSamples(dim); 
		if( colsamp.size() == 1 ) {
			if( input.getColType(colsamp.get(0)) == ColType.METADATA ) {
				
				return input.getMetaData(colsamp.get(0));
			}
		}
		return null;
	}
	...
	{
		String[] allMetaData = getMetaData(dim);
		if (allMetaData != null && allMetaData.length > point_idx)
		{
			return allMetaData[point_idx];
		}
		return null;
	}
