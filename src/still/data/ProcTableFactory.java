...
public class ProcTableFactory extends TableFactory
{
											 ...
											 ) 
	{
		...
		boolean bDebugTestR = false; // just a initial proof of concept test. will remove this later.
		if (bDebugTestR && MathLibs.getREngine() != null)
		{
	        Integer iNumTotal = new Integer(iDataSize + iNumOutliers);
			for( int d = 0; d < iDataDim; d++ )
	    	{
		        REXP result = MathLibs.getREngine().eval("rnorm("+iNumTotal.toString()+",mean=0,sd=1)");
	//	        //REXP result = reng.eval("hist(rnorm(100,mean=0,sd=1))");
		        double[] dres = result.asDoubleArray();
	...
	public static class ProcPanel extends JPanel implements ActionListener
	{
		...
		public ProcPanel(MemoryTable intTab ) {
			...
			m_SpinnerDataDim    = new JSpinner(new SpinnerNumberModel(intTab.columns(), 1, 1000, 1));
