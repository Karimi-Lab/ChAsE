...
public class ClusterTable
{
	...
	/** 
	 * Caches additional information for a cluster, such as the number of points, centroid, ... 
	 */
	public class ClusterInfo
	{
		/** Number of points within this cluster */
		public int 		m_iNumPoints = 0;
		
		/** Cluster centroid. size:[iDataDim] */
		public double 	m_dCentroid[] = null;
		
		/** Quality measures for each cluster. size:[QualityMeasure.size]	 */
		...
		ClusterInfo(ClusterInfo rhs)
		{
			...
			m_QualityMeasuresArray 	= rhs.m_QualityMeasuresArray.clone();
	...
	/**
	 * Caches the cluster info for all clusters of a specific K (number of clusters) 
	 */
	public class AllClusterInfo
	{
		/** Array of cluster info per each cluster. size: [iNumClusters]*/
		public ClusterInfo[]	m_ClusterInfoArray		= null;
		
		/** overall quality measures for the clustering of this  K. size:[QualityMeasure.size]	 */
		public double[] 	  	m_QualityMeasuresArray	= null; 
		
		/** cached cluster id for each point for this K. size: [iDataSize] */
	...
	/** Caches all cluster info for the range of all Ks. size:[m_iMaxNumClusters] */
...
//		SPEARMAN			// Use Spearman Correlation to cluster together genes whose expression profiles have similar shapes or show similar general trends (e.g. increasing expression with time), but whose expression levels may be very different.
	...
	public double[] getInputPoint( int point_idx )
	{
		...
		int idim = getInputDim();
		double[] res = new double[idim];
		for (int d = 0; d < idim; d++)
			res[d] = m_InputTable.getMeasurement(point_idx, d);
		return res;
	}
	...
	public void updateClusters()
	{
		...
		if (m_bAutoCalculateAll)
		{
			...
			{
				calculateAll(m_iMaxK);
			}
		}
		
		if (m_bSelectBestK)
		{
			m_iCurrentK = getBestK(m_Quality);
		}
	}
	...
	public void runKMeans()
	{
		...
		m_ClusterInfoArray = new ClusterInfo[m_iCurrentK + 1]; // last one for outliers
	    ...
	    if (m_AllClusterInfoArray != null && m_AllClusterInfoArray.length >= m_iCurrentK)
	    { // get the centroids from the precalculated list
			for (int c = 0; c < m_iCurrentK; c++)
			{
				m_ClusterInfoArray[c] = new ClusterInfo(m_AllClusterInfoArray[m_iCurrentK - 1].m_ClusterInfoArray[c]);
			}
			m_ClusterInfoArray[m_iCurrentK] = new ClusterInfo(iDataDim);
	    }
	    else
	    {
	    ...
	    while(bClustersChanged && iIterations++ < MAX_ITERATIONS)
	    {
	    	...
	    	for (int idata = 0; idata < iDataSize; idata++)
	    	{
	    		if (isOutlier(idata))
	    		{
	    			m_iClusterIDArray[idata] = m_iCurrentK;
	    			continue;
	    		}
...
//			m_ClusterIDs = d_Clusters;
	...
	public void calculateAll(int iMaxNumClusters)
	{
		int iDataDim = getInputDim();
	    int iDataSize = getInputSize();
		int iCurrentNumClusters = m_iCurrentK;
		
		AllClusterInfo oldAllClusterInfoArray[] = m_AllClusterInfoArray;
		m_AllClusterInfoArray = null;
		
		//TODO: (performance) Don't recreate this array if already in good state. 
		...
		for (int k = 0; k < iMaxNumClusters; k++)
		{
			...
			for (int iM = 0; iM < QualityMeasure.values().length; iM++)
	  		{
				for (int c = 0; c < newAllClusterInfoArray[k].m_ClusterInfoArray.length; c++)
					newAllClusterInfoArray[k].m_ClusterInfoArray[c].m_QualityMeasuresArray[iM] = 0;
			...
			{// calculate the quality measures
	    		...
	    		int idataClusterID = m_iClusterIDArray[idata];
	    		double dEuclDist = getDist(getInputPoint(idata), m_ClusterInfoArray[idataClusterID].m_dCentroid, DistanceMetric.EUCLIDEAN_DISTANCE);
	    		
	    		double dIntraClusterDist = dEuclDist / m_ClusterInfoArray[idataClusterID].m_iNumPoints;
	    		newAllClusterInfoArray[k].m_ClusterInfoArray[idataClusterID].m_QualityMeasuresArray[QualityMeasure.INTRA_CLUSTER_DIST.ordinal()] +=	dIntraClusterDist;
	    		newAllClusterInfoArray[k].m_QualityMeasuresArray[QualityMeasure.INTRA_CLUSTER_DIST.ordinal()] += dIntraClusterDist;
	    		
	    		double dSumSquaredError = (dEuclDist * dEuclDist) * m_iCurrentK; 
	    		newAllClusterInfoArray[k].m_ClusterInfoArray[idataClusterID].m_QualityMeasuresArray[QualityMeasure.SUM_SQUARED_ERROR.ordinal()] += dSumSquaredError;
	    		newAllClusterInfoArray[k].m_QualityMeasuresArray[QualityMeasure.SUM_SQUARED_ERROR.ordinal()] += dSumSquaredError;
			}
			
			if (oldAllClusterInfoArray != null && k < oldAllClusterInfoArray.length)
			{// use the old clustering, if the new one has a worst score.
				if (oldAllClusterInfoArray[k].m_QualityMeasuresArray[m_Quality.ordinal()] <
					newAllClusterInfoArray[k].m_QualityMeasuresArray[m_Quality.ordinal()])
				{
		...
		m_iCurrentK = iCurrentNumClusters;
		runKMeans();
	}
  ...
  public void calculateNeighbors(double dMaxNeighborDist)
  {
	...
	static final int NEIGHBOR_DIST_STEPS = 300; // determines the resolution
